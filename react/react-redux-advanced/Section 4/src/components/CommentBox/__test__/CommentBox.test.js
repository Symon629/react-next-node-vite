...
it("contains a textarea and a button", () => {
    ...
    // CommentBox now renders two <button> elements (Submit Comment and
    // Fetch Comments), so the bare `getByRole("button")` query throws
    // "Found multiple elements". We narrow each lookup by its visible
    // text via the `name` option (matched as an accessible name).
    expect(screen.getByRole("button", { name: /submit comment/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /fetch comments/i })).toBeInTheDocument();
});
...
it("contains a textarea and a button", () => {
    ...
    expect(wrapper.find("button").length).toEqual(2); // one for submit, one for fetch
});
